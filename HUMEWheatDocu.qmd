...
if (EC.v >= 40) and (NNI60.v <= 0) and 
  if $ln(TOPWT_{m2} \cdot Q45) > 0$ then 
...
The value of QHI is continuously calculated/updated from BBCH 60 to BBCH 75 as the ratio of sum of accumulated radiation to the sum of accumulated temperature since \>= BBCH 60.
...
The derivation of this equation is based on equation determining the time of the inflection point of a logistic growth function t_{inf} here called X~mid~ and the assumption that the maximum value of RHI is 1:

$$t_{inf}​= \frac{1}{r_{RHI}} \cdot ln \left( \frac{1−RHI_{ini}}{RHI_{ini}} \right)$$ {#eq-tinf}

Solving equation @eq-tinf for r_{RHI} yields equation @eq-rRHI.
...
$$LA{{I}_{new}}(t)=\max (0,{{W}_{leaf}}(t)\times SLA_{pot}(t) \cdot 10^{-4} - LAI(t))$$ {#eq-LAI_new}
...
At the beginning of leaf growth SLApot is initialized with the maximum SLA (SLAmax). Two hypothetical SLA values (SLALAI, SLATrans) are calculated to describe the further SLA development. The first describes SLA with respect on mutual shading as a function of LAI.

$$SLA_{LAI}(t)=a_{SLA}+b_{SLA} \cdot LAI(t)$$ {#eq-SLALAI}

The second describes the transition phase between SLAmax and SLALAI:

$$SLA_{T\text{rans}}}(t)=SL{{A}_{LAI}}(t)+ \\ 
 & \mathop{{}}^{{}}\mathop{{}}^{{}}\mathop{{}}^{{}}(SL{{A}_{\max }}-SL{{A}_{LAI}}(t))\cdot \exp (fdec1\cdot LAI(t)+fdec2) \\ 
$$
